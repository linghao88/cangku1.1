...
class ProcurementSystem:
    ...
    def _show_splash_and_load(self):
        ...
        if total_items > 5000:
            ...
            self.root.after(50, lambda: self._async_load_data(total_items))
        else:
            self.root.after(50, lambda: self._async_load_data(total_items, show_progress=False))

    def _async_load_data(self, total_items, show_progress=True):
        """异步加载核心逻辑"""
        try:
            # 阶段1：加载供应商数据
            ...
            final_actions = [
                ...
                lambda: self.root.deiconify()  # 确保主窗口在前
            ]
            if show_progress:
                final_actions.insert(0, self.progress_dialog.close)
            self.root.after(0, lambda: [action() for action in final_actions])

        except Exception as e:
            error_actions = [
                lambda: messagebox.showerror("错误", f"加载失败: {str(e)}", parent=self.root)
            ]
            if show_progress:
                error_actions.insert(0, self.progress_dialog.close)
            self.root.after(0, lambda: [action() for action in error_actions])
